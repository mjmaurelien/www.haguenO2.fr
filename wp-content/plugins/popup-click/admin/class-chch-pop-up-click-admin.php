...
if ( !class_exists( 'ChChPUCFPeview' ) )
  require_once ( CHCH_PUCF_PLUGIN_DIR . 'admin/includes/chch-pucf-preview.php' );

if ( !class_exists( 'ChChPCFTemplate' ) )
  require_once ( CHCH_PUCF_PLUGIN_DIR . 'public/includes/chch-pucf-template.php' );

if ( file_exists( CHCH_PUCF_PLUGIN_DIR . 'admin/includes/CMB2/init.php' ) ) {
  require_once CHCH_PUCF_PLUGIN_DIR . 'admin/includes/CMB2/init.php';
}
...
class ChChPopUpClickAdmin {
  ...
  function __construct() {
    ...
    add_action( 'init', array( $this, 'chch_pucf_register_post_type' ) );
    // Customize the columns in the popup list.
    add_filter( 'manage_chch-pucf_posts_columns', array( $this, 'chch_pucf_custom_pop_up_columns' ) );
    // Returns the content for the custom columns.
    add_action( 'manage_chch-pucf_posts_custom_column', array( $this, 'chch_pucf_manage_custom_pop_up_columns' ), 10, 2 );
    add_filter( 'post_updated_messages', array( $this, 'chch_pucf_post_type_messages' ) );
    ...
    // Templates view
    add_action( 'edit_form_after_title', array( $this, 'chch_pucf_templates_view' ) );

    // admin scripts and AJAX
    add_action( 'admin_init', array( $this, 'chch_pucf_tinymce_event' ) );
    add_action( 'admin_head', array( $this, 'chch_pucf_admin_head_scripts' ) );
    add_action( 'admin_enqueue_scripts', array( $this, 'chch_pucf_enqueue_admin_scripts' ) );
    add_action( 'wp_ajax_chch_pucf_load_preview_module', array( $this, 'chch_pucf_load_preview_module' ) );
   ...
   * @since    0.1.0
   */
  public function chch_pucf_register_post_type() {

    $domain = $this->plugin_slug;

    $labels = array(
      'name' => _x( 'Pop-Up CC Click Free', 'Post Type General Name', $domain ),
      'singular_name' => _x( 'Pop-Up Click Free', 'Post Type Singular Name', $domain ),
      'menu_name' => __( 'Pop-Up CC Click Free', $domain ),
      'parent_item_colon' => __( 'Parent Item:', $domain ),
      'all_items' => __( 'Pop-Ups CC Click Free', $domain ),
      'view_item' => __( 'View Item', $domain ),
      'add_new_item' => __( 'Add New Pop-Up Click Free', $domain ),
      'add_new' => __( 'Add New Pop-Up Click Free', $domain ),
      'edit_item' => __( 'Edit Pop-Up Click Free', $domain ),
      'update_item' => __( 'Update Pop-Up Click Free', $domain ),
      'search_items' => __( 'Search Pop-Up Click Free', $domain ),
      'not_found' => __( 'Not found', $domain ),
      'not_found_in_trash' => __( 'No Pop-Up Close found in Trash', $domain ),
      );

    $args = array(
      'label' => __( 'Pop-Up Click Free', $domain ),
      ...
      'rewrite' => false );
    register_post_type( 'chch-pucf', $args );
  }
   ...
   *
   */
  function chch_pucf_custom_pop_up_columns( $defaults ) {
    $defaults['chch_pucf_status'] = __( 'Active', $this->plugin_slug );
    $defaults['chch_pucf_template'] = __( 'Template', $this->plugin_slug );
   ...
   * @since     1.0.0
   */
  function chch_pucf_manage_custom_pop_up_columns( $column, $post_id ) {
    global $post;
    if ( $column === 'chch_pucf_status' ) {
      echo ucfirst( get_post_meta( $post_id, '_chch_pucf_status', true ) );
    }

    if ( $column === 'chch_pucf_template' ) {
      echo ucfirst( get_post_meta( $post_id, '_chch_pucf_template', true ) );
    }
  }
   ...
   */
  function chch_pucf_post_type_messages( $messages ) {
    $post = get_post();
    $post_type = get_post_type( $post );
    $post_type_object = get_post_type_object( $post_type );

    $messages['chch-pucf'] = array(
   ...
   * @since  0.1.0
   */
  function chch_pucf_initialize_cmb_meta_boxes() {
    $domain = $this->plugin_slug;
    $prefix = '_chch_pucf_';
    ...
    $general_metabox = new_cmb2_box( array(
      'id' => 'chch-pucf-metabox-general',
      'title' => __( 'GENERAL', $domain ),
      'object_types' => array( 'chch-pucf' ),
      'priority' => 'low',
      ) );
    ...
    $general_metabox->add_field( array(
      'name' => __( 'Autoplay video?', $domain ),
      'desc' => __( 'Pop-up video will start playing when the pop-up is opened.', $domain ),
   ...
   *
   */
  function chch_pucf_render_cookie_select( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
    ...
    endforeach;
    echo '</select> <a href="http://ch-ch.org/popuppro" class="getpro" target="_blank">Get Pro</a>';
   ...
   *
   */
  function chch_pucf_render_pages_select( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
   ...
   * @since  0.1.0
   */
  public function chch_pucf_metabox( $post ) {
    remove_meta_box( 'slugdiv', 'chch-pucf', 'normal' );

    add_meta_box( 'chch-pucf-metabox-click', __( 'Click Options', $this->plugin_slug ), array( $this, 'chch_pucf_render_click_metabox' ), 'chch-pucf', 'normal', 'high' );

    $post_boxes = array(
      'chch-pucf-metabox-general',
      'chch-pucf-metabox-content',
      'chch-pucf-metabox-control',
      'chch-pucf-metabox-click',
      );
   ...
   * @since  1.0.0
   */
  function chch_pucf_add_metabox_classes( $classes ) {
    array_push( $classes, "cc-pu-tab-2 cc-pu-tab" );
    return $classes;
  }

  /**
   * View for Click Metabox
   *
   * @since  1.0.0
   */
  function chch_pucf_render_click_metabox( $post ) {
    require_once ( CHCH_PUCF_PLUGIN_DIR . 'admin/views/metabox/click-metabox.php' );
  }
   ...
   * @since  0.1.0
   */
  function chch_pucf_save_pop_up_meta( $post_id, $post, $update ) {
    if ( !isset( $_POST['chch_pucf_save_nonce'] ) || !wp_verify_nonce( $_POST['chch_pucf_save_nonce'], 'chch_pucf_save_nonce_' . $post_id ) ) {
      return;
    }
    ...
    $template = $_REQUEST['_chch_pucf_template'];
    $template_base = $_REQUEST['_chch_pucf_template_base'];

    update_post_meta( $post_id, '_chch_pucf_template', sanitize_text_field( $_REQUEST['_chch_pucf_template'] ) );
    update_post_meta( $post_id, '_chch_pucf_template_base', sanitize_text_field( $_REQUEST['_chch_pucf_template_base'] ) );

    if ( !empty( $template ) && !empty( $template_base ) ) {
      $template_data = array();

      if ( isset( $_REQUEST['_' . $template . '_size_custom'] ) ) {
        $template_data['size'] = array( 'custom' => 1 );
      } else {
        $template_data['size'] = array( 'custom' => 0 );
      }
      ...
      $template_data['border'] = array( 'radius' => $_REQUEST['_' . $template . '_border_radius'] );

      $template_data['overlay'] = array( 'hide' => $_REQUEST['_' . $template . '_overlay_hide'] );

      $p_array = array( '</p>', '<p>' );

      $template_data['contents'] = array( 'desc' => wp_kses_post( $_REQUEST['_' . $template . '_contents_desc'] ), 'video' => $_REQUEST['_' . $template . '_contents_video'] );

      update_post_meta( $post_id, '_chch_pucf_item', sanitize_text_field( $_REQUEST['_chch_pucf_item'] ) );
   ...
   * @since  0.1.0
   */
  public function chch_pucf_templates_view( $post ) {

    $screen = get_current_screen();
    if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type ) {

      include ( CHCH_PUCF_PLUGIN_DIR . 'admin/views/templates.php' );
    }
  }
  ...
  public function get_templates() {
    if ( !class_exists( 'PluginMetaData' ) )
      require_once ( CHCH_PUCF_PLUGIN_DIR . 'admin/includes/PluginMetaData.php' );
    $pmd = new PluginMetaData;
    $pmd->scan( CHCH_PUCF_PLUGIN_DIR . 'public/templates' );
    return $pmd->plugin;
  }
   ...
   *
   */
  function chch_pucf_tinymce_event() {
    if ( is_admin() && current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
      if ( get_bloginfo( 'version' ) < 3.9 ) {
        add_filter( 'mce_external_plugins', array( $this, 'chch_pucf_tinymce_keyup_old' ) );
      } else {
        add_filter( 'mce_external_plugins', array( $this, 'chch_pucf_tinymce_keyup' ) );
      }
    }
  }
   ...
   *
   */
  function chch_pucf_tinymce_keyup( $plugin_array ) {
    $plugin_array['keyup_events'] = CHCH_PUCF_PLUGIN_URL . 'admin/assets/js/chch-tinymce.js';
    return $plugin_array;
  }
   ...
   *
   */
  function chch_pucf_tinymce_keyup_old( $plugin_array ) {
    $plugin_array['keyup_events'] = CHCH_PUCF_PLUGIN_URL . 'admin/assets/js/chch-tinymce-old.js';
    return $plugin_array;
  }
   ...
   * @since  0.1.0
   */
  public function chch_pucf_admin_head_scripts() {
    $screen = get_current_screen();
    if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type ) {
   ...
   *
   */
  public function chch_pucf_enqueue_admin_scripts() {
    ...
    if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type ) {
      ...
      wp_enqueue_style( $this->plugin_slug . '-admin-styles', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), ChChPopUpClick::VERSION );
   ...
   *
   */
  function chch_pucf_load_preview_module() {

    $template = $_POST['template'];
    $template_base = $_POST['base'];
    $popup = $_POST['id'];

    $template = new ChChPCFTemplate( $template, $template_base, $popup );
