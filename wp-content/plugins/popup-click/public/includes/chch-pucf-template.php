...
class ChChPCFTemplate {
	...
	function build_js() {
		...
		if(get_post_meta($id, '_chch_pucf_autoplay_video',true))
		{
			$script .= 'var object_iframe = $( "#modal-'.$id.' .modal-inner" ).find("iframe");';
			$script .= 'var object_iframe_src = object_iframe.attr("src");';
		}
		$script .= 'clicklEl.on("click touch", function(e){
						e.preventDefault();';

		if(get_post_meta($id, '_chch_pucf_autoplay_video',true))
		{
			$script .= 'if (object_iframe.length > 0) {
		...
		if(get_post_meta($id, '_chch_pucf_autoplay_video',true))
		{
			$script .= '$(".cc-pucf-close").click(function() {
							setTimeout(function () {
